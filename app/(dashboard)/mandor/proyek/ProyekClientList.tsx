...
import { Search, FolderKanban, ImageIcon } from "lucide-react";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sortedProjects.map((project) => (
            <Link key={project.id} href={`/mandor/proyek/${project.id}`}>
              <Card
                hover
                className="h-full group border-0 shadow-md hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 bg-gradient-to-br from-white to-slate-50 overflow-hidden"
              >
                {/* Project Image */}
                  ...
                  <div className="absolute top-3 left-3">
                    <span
                      className={`${getStatusBadge(
                        project.status
                      )} text-xs font-semibold px-3 py-1.5 rounded-lg inline-block shadow-md`}
                    >
                      {project.status}
                    </span>
                  </div>
