...
const getStatusBadgeVariant = (status: string): "success" | "info" | "danger" | "warning" | undefined => {
  switch (status) {
    case 'Selesai':
      return 'success';
    case 'Dalam Progress':
...
async function getDashboardStats() {
  const [
    ...
    db.select({ value: count() }).from(projeks).where(eq(projeks.status, 'Selesai')),
    db.select({ value: count() }).from(projeks).where(eq(projeks.status, 'Dalam Progress')),
    db.select({ value: count() }).from(projeks).where(eq(projeks.status, 'Perencanaan')),
    db.select({ value: count() }).from(projeks).where(eq(projeks.status, 'Dibatalkan')),
    db.select({ value: count() }).from(portfolios),
    db.select({ value: count() }).from(testimonis).where(eq(testimonis.approved, false)),
  ]);
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <Link href="/admin/proyek/tambah" className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-accent transition-colors">
                <Plus className="w-4 h-4 text-primary" />
                <span className="font-medium">Tambah Proyek Baru</span>
              </Link>
              
              <Link href="/admin/testimoni" className="flex items-center justify-between p-3 rounded-lg border border-border hover:bg-accent transition-colors">
              ...
              <Link href="/admin/pengguna" className="flex items-center justify-between p-3 rounded-lg border border-border hover:bg-accent transition-colors">
                <div className="flex items-center gap-3">
                    ...
                    <li className="text-center p-8">
                      <div className="flex flex-col items-center gap-3 text-muted-foreground">
                        ...
                        <div>
                          ...
                          <p className="text-sm mt-1">Proyek yang dibuat akan muncul di sini</p>
                        </div>
              ...
              {stats.recentProjects.length > 0 && (
                <div className="border-t border-border p-4">
                  <Button variant="ghost"   className="w-full justify-center">
                    <Link href="/admin/proyek" className="flex items-center gap-2">
