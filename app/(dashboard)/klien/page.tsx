        ...
        {klienProjects.length > 0 ? (
              ...
              >
                <Card className="h-full border border-gray-200 shadow-sm hover:border-blue-300 hover:shadow-md transition-all duration-200 cursor-pointer overflow-hidden relative">
                  ...
                  <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${
                    proyek.status === 'Dalam Progress' ? 'bg-blue-500' : 
                    proyek.status === 'Selesai' ? 'bg-green-500' : 'bg-gray-300'
                  }`} />
                  ...
                  <CardContent className="p-6 pl-8">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 text-sm text-gray-500">
                          ...
                          <span className="truncate max-w-[200px]">{proyek.alamat}</span>
                        </div>
                      </div>
                      <Badge className={`px-3 py-1 ${getStatusColor(proyek.status)}`}>
                        {proyek.status}
                      </Badge>
                    </div>
                    ...
                    <div className="mb-5">
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="font-bold text-gray-900">{proyek.progress || 0}%</span>
                      </div>
                      <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-500 ${
                            proyek.progress === 100 ? 'bg-green-500' : 'bg-blue-600'
                          }`}
                    ...
                    <div className="pt-4 border-t border-gray-100 flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-2 text-gray-500 text-xs">
                        ...
                        Update: {new Date(proyek.lastUpdate || proyek.updatedAt).toLocaleDateString('id-ID')}
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-gray-300">
            ...
            <p className="text-gray-500 max-w-sm mx-auto mt-2">
              Saat ini Anda belum memiliki proyek yang aktif. Hubungi admin untuk memulai proyek baru.
            </p>
          </div>
        )}
      </section>
